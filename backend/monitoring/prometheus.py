...
# Custom metrics (only if prometheus is available)
if PROMETHEUS_AVAILABLE:
    ...
    REQUEST_COUNT = ACTIVE_USERS = CHAT_MESSAGES = CALCULATOR_USAGE = None
    CPU_USAGE = MEMORY_USAGE = DISK_USAGE = None

def setup_prometheus(app):
    """Setup Prometheus metrics for FastAPI"""
    if not PROMETHEUS_AVAILABLE:
        print("⚠️ Prometheus not available, skipping metrics setup")
        return None
...
def update_system_metrics():
    ...
    if PROMETHEUS_AVAILABLE and CPU_USAGE:
...
def track_chat_message():
    ...
    if PROMETHEUS_AVAILABLE and CHAT_MESSAGES:
        CHAT_MESSAGES.inc()

def track_calculator_usage(calculator_type: str):
    """Track calculator usage"""
    if PROMETHEUS_AVAILABLE and CALCULATOR_USAGE:
