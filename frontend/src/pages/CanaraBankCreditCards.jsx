...
export default function CanaraBankCreditCards() {
  const navigate = useNavigate();
  const [selectedCategory, setSelectedCategory] = useState('all');

  const canaraCards = [
    {
      name: 'Canara Platinum Card',
      type: 'Platinum Card',
      category: 'Salaried',
      annualFee: '₹750',
      features: [
  ...
  const getFilteredCards = () => {
    if (selectedCategory === 'all') return canaraCards;
    return canaraCards.filter(card => card.category === selectedCategory);
  };
      ...
      <div className="bg-gradient-to-r from-blue-600 to-purple-600 text-white">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4">
          ...
          <div className="text-center">
            ...
            <h1 className="text-2xl md:text-3xl font-bold mb-2">Canara Bank Credit Cards</h1>
            <p className="text-base opacity-90">Explore Canara Bank's range of credit cards</p>
          </div>
        </div>
      </div>
