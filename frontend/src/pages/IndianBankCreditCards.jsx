...
export default function IndianBankCreditCards() {
  const navigate = useNavigate();
  const [selectedCategory, setSelectedCategory] = useState('all');

  const indianBankCards = [
    {
      name: 'Indian Bank Platinum Card',
      type: 'Platinum Card',
      category: 'Salaried',
      annualFee: '₹599',
      features: [
  ...
  const getFilteredCards = () => {
    if (selectedCategory === 'all') return indianBankCards;
    return indianBankCards.filter(card => card.category === selectedCategory);
  };
      ...
      <div className="bg-gradient-to-r from-blue-600 to-purple-600 text-white">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4">
          ...
          <div className="text-center">
            ...
            <h1 className="text-2xl md:text-3xl font-bold mb-2">Indian Bank Credit Cards</h1>
            <p className="text-base opacity-90">Explore Indian Bank's range of credit cards</p>
          </div>
        </div>
      </div>
