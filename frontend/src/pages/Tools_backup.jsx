...
import TaxSaverCalc from '../calculators/TaxSaverCalc';
...
export default function Tools() {
  ...
  // Legacy toolCategories (kept for backward compatibility)
  const toolCategories = [
    ...
    },
    {
      ...
      tools: [
        { name: 'RD Calculator', icon: CalculatorIcon, component: RDCalculator, description: 'Calculates maturity and total interest from Recurring Deposits' },
        { name: 'PPF Calculator', icon: CalculatorIcon, component: PPFCalculator, description: 'Calculates maturity value of Public Provident Fund' },
        { name: 'Post Office FD Calculator', icon: CalculatorIcon, component: PostOfficeFDCalculator, description: 'For rural/senior-friendly schemes like Monthly Income Scheme' }
      ]
    },
    {
      name: 'Investment Tools',
      tools: [
        { name: 'Step-up SIP Calculator', icon: ChartBarIcon, component: StepUpSIPCalculator, description: 'Calculates returns if SIP amount increases yearly' },
        { name: 'Lumpsum Investment Calculator', icon: CurrencyDollarIcon, component: LumpsumInvestmentCalculator, description: 'Calculates one-time investment returns (with CAGR)' },
        { name: 'Goal-Based Investment Planner', icon: ChartBarIcon, component: GoalBasedInvestmentPlanner, description: 'Helps users invest monthly to reach future goals' },
        { name: 'Mutual Fund Return Tracker', icon: ChartBarIcon, component: MutualFundReturnTracker, description: 'Estimates mutual fund returns over a period' },
        { name: 'Gold Investment Calculator', icon: CurrencyDollarIcon, component: GoldInvestmentCalculator, description: 'SGB/digital gold returns over years' }
      ]
    },
    {
      name: 'Loan & Credit Tools',
      tools: [
        { name: 'Home Loan EMI Calculator', icon: CalculatorIcon, component: HomeLoanEMICalculator, description: 'Estimate EMI + interest breakup' },
        { name: 'Education Loan EMI Calculator', icon: CalculatorIcon, component: EducationLoanEMICalculator, description: 'Helps students plan repayment' },
        { name: 'Car/Bike Loan EMI Calculator', icon: CalculatorIcon, component: CarBikeLoanEMICalculator, description: 'For vehicle buyers' },
        { name: 'Credit Card EMI Calculator', icon: CalculatorIcon, component: CreditCardEMICalculator, description: 'Helps understand EMI on card purchases' },
        { name: 'Loan Eligibility Checker', icon: CalculatorIcon, component: LoanEligibilityChecker, description: 'Based on salary/income, debt ratio' },
        { name: 'Loan Affordability Tool', icon: CalculatorIcon, component: LoanAffordabilityTool, description: 'Shows how much EMI one can afford monthly' }
      ]
    },
    {
      name: 'Insurance Tools',
      tools: [
        { name: 'Term Life Insurance Estimator', icon: CalculatorIcon, component: TermLifeInsuranceEstimator, description: 'Estimates ideal life cover based on income & age' },
        { name: 'Health Insurance Premium Estimator', icon: CalculatorIcon, component: HealthInsurancePremiumEstimator, description: 'Based on city, age, coverage amount' },
        { name: 'Vehicle Insurance Estimator', icon: CalculatorIcon, component: VehicleInsuranceEstimator, description: 'Basic premium for private cars/bikes' },
        { name: 'Crop Insurance Estimator', icon: CalculatorIcon, component: CropInsuranceEstimator, description: 'For rural audience (optional, Bharat-focused)' }
      ]
    },
    {
      name: 'Retirement Tools',
      tools: [
        { name: 'NPS Return Calculator', icon: ChartBarIcon, component: NPSReturnCalculator, description: 'Calculates future value based on investment %' },
        { name: 'Retirement Goal Planner', icon: ChartBarIcon, component: RetirementGoalPlanner, description: 'Helps users invest monthly for post-60 life' },
        { name: 'Annuity Calculator', icon: CurrencyDollarIcon, component: AnnuityCalculator, description: 'Post-retirement income based on lump sum corpus' },
        { name: 'EPF Maturity Calculator', icon: CalculatorIcon, component: EPFMaturityCalculator, description: 'For salaried employees (estimate maturity at 58)' }
      ]
    },
    {
      name: 'Credit & Score Tools',
      tools: [
        { name: 'Credit Score Simulator', icon: ChartBarIcon, description: 'Simulates how score changes based on EMI defaults, usage' },
        { name: 'Debt Repayment Planner', icon: CalculatorIcon, component: DebtRepaymentPlanner, description: 'Strategizes how to repay loans faster using snowball/avalanche method' }
      ]
    },
    {
      name: 'Tax & Salary Tools',
      tools: [
        { name: 'HRA Exemption Calculator', icon: CalculatorIcon, component: HRAExemptionCalculator, description: 'Show HRA deduction based on salary & rent' },
        { name: 'Form 16 Breakdown Tool', icon: DocumentTextIcon, component: Form16BreakdownTool, description: 'Explains salary components to employees' },
        { name: 'TDS Estimator', icon: CalculatorIcon, component: TDSEstimator, description: 'For freelancers, contract workers, creators' }
      ]
    },
    {
      name: 'Personal Planning Tools',
      tools: [
        { name: 'Net Worth Tracker', icon: ChartBarIcon, component: NetWorthTracker, description: 'Calculates total assets – liabilities' },
        { name: 'Emergency Fund Calculator', icon: CurrencyDollarIcon, component: EmergencyFundCalculator, description: 'Shows how much savings needed for 3/6 months' },
        { name: '50/30/20 Rule Budgeter', icon: DocumentTextIcon, component: BudgetRuleCalculator, description: 'Income-based auto-planner' },
        { name: 'First Salary Planner', icon: CalculatorIcon, component: FirstSalaryPlanner, description: 'For freshers to plan savings, tax, and spend smartly' }
      ]
    },
    {
      name: 'Specialized Tools',
      tools: [
        { name: 'Scholarship Eligibility Tool', icon: DocumentTextIcon, component: ScholarshipEligibilityTool, description: 'Based on marks, income, state – India-focused' },
        { name: 'Rent vs Buy Home Calculator', icon: CalculatorIcon, component: RentVsBuyCalculator, description: 'Compares renting vs buying a house over years' },
        { name: 'Real Returns Calculator', icon: ChartBarIcon, component: RealReturnsCalculator, description: 'Adjusts returns for inflation impact' },
        { name: 'Currency Conversion Tool', icon: CurrencyDollarIcon, description: 'For NRI & Forex planning' },
        { name: 'Education Cost Estimator', icon: CalculatorIcon, component: EducationCostEstimator, description: 'Plan for future cost of school/college fees' }
      ]
    }
  ];
  ...
  }, [searchTerm, selectedLevel, selectedTag, selectedCategory, favoriteTools, recentTools]);
  ...
  return (
    <div className="py-12 min-h-screen">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
              ...
              <div className="flex flex-wrap gap-2 mt-4 pt-4 border-t border-white/10">
                ...
                {selectedLevel !== 'All' && (
                  <span className="bg-green-500/20 text-green-300 px-2 py-1 rounded text-xs">
                    {getLevelColor(selectedLevel)} {selectedLevel}
                  </span>
                )}
                {selectedTag !== 'All' && (
                  <span className="bg-purple-500/20 text-purple-300 px-2 py-1 rounded text-xs capitalize">
                    {selectedTag}
                  </span>
                )}
          ...
          {selectedCategory === 'Recent' && recentToolsList.length > 0 && (
            <div className="mb-12">
              <div className="flex items-center gap-3 mb-6">
                <span className="text-2xl">🕒</span>
                <h2 className="text-2xl font-bold text-white">Recently Used Tools</h2>
                <span className="text-sm text-white/60">Continue where you left off</span>
              </div>
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {recentToolsList.map(tool => renderToolCard(tool))}
              </div>
            </div>
          )}
          ...
          {!searchTerm && selectedLevel === 'All' && selectedTag === 'All' && selectedCategory === 'All' && (
            <div className="mb-12">
              <div className="flex items-center gap-3 mb-6">
                ...
                <span className="text-sm text-white/60">Most used by our community</span>
              </div>
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          ...
          {selectedCategory === 'Recent' && recentToolsList.length === 0 && (
            <div className="mb-12">
              <div className="glass-card text-center py-12">
                ...
                <h3 className="text-xl font-semibold text-white mb-2">No Recent Tools</h3>
                <p className="text-white/60 mb-4">
                  Start using tools to see them appear here for quick access.
                </p>
                <button
          ...
          {selectedCategory === 'Favorites' && favoriteToolsList.length === 0 && (
            <div className="mb-12">
              <div className="glass-card text-center py-12">
                <HeartIcon className="h-16 w-16 text-white/30 mx-auto mb-4" />
                <h3 className="text-xl font-semibold text-white mb-2">No Favorite Tools Yet</h3>
                <p className="text-white/60 mb-4">
