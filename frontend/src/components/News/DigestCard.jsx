...
const DigestCard = () => {
  ...
  const fetchDigest = async () => {
    ...
    setError(null);
    try {
      const response = await fetch('http://localhost:8001/api/news/daily-digest');
  ...
  if (loading) {
    return (
      <div className="bg-gradient-to-br from-blue-50 to-purple-50 rounded-xl border border-blue-100 p-6 animate-pulse">
  ...
  return (
    <div className="bg-gradient-to-br from-blue-50 to-purple-50 rounded-xl border border-blue-100 p-6 mb-8">
      ...
      {/* Date */}
      <p className="text-sm text-blue-700 mb-4 flex items-center">
      ...
      <div className="bg-white/70 rounded-lg p-4 mb-4 border border-blue-200">
        <div className="flex items-center mb-2">
          ...
          <span className="text-sm font-medium text-blue-800">AI Overview</span>
        </div>
      ...
      {digest.top_news && digest.top_news.length > 0 && (
        <div>
          <h4 className="text-sm font-medium text-blue-800 mb-3">Top Headlines ({digest.total_count})</h4>
          <div className="space-y-2">
            {digest.top_news.slice(0, 5).map((news, index) => (
              <div key={index} className="bg-white/50 rounded-lg p-3 border border-blue-100">
